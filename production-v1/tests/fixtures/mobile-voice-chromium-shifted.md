...
`mobile-voice-chromium-shifted.wav` is a synthetic pinned-Chromium microphone capture from `mobile-voice-en.wav` with the test-only challenge seed `Buffer.alloc(32, 0x31)`. Captured on 2026-09-08 using the existing fake microphone browser flow; it contains no real microphone/user recording.
